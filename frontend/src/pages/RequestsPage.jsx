...
function InsightCards({ analytics, total, scenario, t, lang }) {
  ...
  if (total > 0) {
    ...
    if (pendingPct > 0) {
      insights.push({ icon: INSIGHT_ICONS.clock, color: "amber", text: `${pendingPct}% (${newCount}) ${t.insightPending || "ожидают обработки"}`, sub: t.insightPendingHint || "Новые заявки без статуса" });
    }
    if (doneCount > 0) {
      insights.push({ icon: INSIGHT_ICONS.check, color: "green", text: `${Math.round((doneCount / total) * 100)}% ${t.insightDone || "завершено"}`, sub: `${doneCount} ${t.insightOf || "из"} ${total}` });
    }
    if (hiredCount > 0 || acceptedCount > 0) {
      const convCount = hiredCount + acceptedCount;
      insights.push({ icon: INSIGHT_ICONS.star, color: "blue", text: `${Math.round((convCount / total) * 100)}% ${t.insightConversion || "конверсия"}`, sub: `${convCount} ${t.insightAccepted || "принято / нанято"}` });
    }
    if (rejectedCount > 0) {
      insights.push({ icon: INSIGHT_ICONS.xmark, color: "red", text: `${rejectedCount} ${t.insightRejected || "отклонено"}`, sub: `${Math.round((rejectedCount / total) * 100)}% ${t.insightFromTotal || "от всех"}` });
    }
  }

  if (analytics.avgPerDay !== "0") {
    insights.push({ icon: INSIGHT_ICONS.trending, color: "teal", text: `${analytics.avgPerDay} ${t.insightAvgDay || "в среднем/день"}`, sub: t.insightAvgDayHint || "За последние 14 дней" });
  }
  if (analytics.uniqueEmails > 0) {
    insights.push({ icon: INSIGHT_ICONS.user, color: "purple", text: `${analytics.uniqueEmails} ${t.insightUnique || "уникальных участников"}`, sub: t.insightUniqueHint || "По email адресам" });
  }
  const dowNames = DOW_LABELS[lang] || DOW_LABELS.ru;
  if (analytics.busiestDow !== undefined) {
    insights.push({ icon: INSIGHT_ICONS.calendar, color: "indigo", text: `${dowNames[analytics.busiestDow]} — ${t.insightBusiestDay || "самый активный день"}`, sub: t.insightBusiestHint || "По количеству заявок" });
  }
  if (analytics.answerDistributions.length > 0) {
    ...
    insights.push({ icon: INSIGHT_ICONS.chat, color: "green", text: `«${topAns}» — ${pct}%`, sub: shortQ });
  }
